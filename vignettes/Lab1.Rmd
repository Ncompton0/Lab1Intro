...
here = t(here)
density <- as.vector(data[["Density"]])
density <- matrix(density, 1, length(density))
Strength_MD = as.vector(data[["Machine Direction"]])
Strength_MD = matrix(Strength_MD, 1, length(Strength_MD))
Strength_CD = as.vector(data[["Cross Direction"]])
Strength_CD = matrix(Strength_CD, 1, length(Strength_CD))
```

# Find Sample Mean, Sample Covarience (biased), and Sample Correlation of T1-2 data
